# Script to calculate ChAs given network of contacts and features
...
PCHiC_wt=PCHiC_map[which(PCHiC_map$mESC_wt>5),]
...
#load mESC chromatin features already matched to PCHiC network fragments (otherwise generate from bedepi, see below)
# agchic=read.table('../testing_data/Features_mESC.txt', sep='\t', row.names=1)
agchic=read.table(args[2], sep='\t', row.names=1)
